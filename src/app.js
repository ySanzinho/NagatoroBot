...
client.on('message', async message => {
      ...
      if (hasLevelUp) {
        ...
        message.channel.send(`Você upou para o nível ${user.level}!`);
      }

  
      if(!message.content.startsWith(prefix)&& message.content.startsWith(client.user.id)) return message.reply(`Meu Prefixo é: **\`${prefix}\`**, digite \`${prefix}help\` para mais informação!`); //if the messages is not a command and someone tags the bot, then send an info msg
